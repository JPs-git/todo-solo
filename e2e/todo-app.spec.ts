...
test.describe('Todo-List 端到端测试', () => {
  ...
  test.describe('核心功能', () => {
    test('TC-001: 正常创建任务', async ({ page }) => {
      ...
      await expect(page.locator('.task-card__title')).toContainText('测试任务1');
    });
    ...
    test('TC-003: 创建长标题任务', async ({ page }) => {
      ...
      await page.click('.btn-primary:has-text("添加任务")');
      await expect(page.locator('.task-card__title')).toContainText(longTitle.substring(0, 50));
    });
    ...
    test('TC-012: 显示空列表状态', async ({ page }) => {
      await expect(page.locator('.empty-state')).toBeVisible();
      await expect(page.locator('.empty-state__title')).toContainText('暂无任务');
    });
    ...
    test('TC-013: 刷新页面数据保持', async ({ page }) => {
      ...
      await page.click('.btn-primary:has-text("添加任务")');
      await page.reload();
      await expect(page.locator('.task-card__title')).toContainText('持久化任务');
    });
  });
  ...
  test.describe('基础功能', () => {
    ...
    test('TC-019: 筛选已完成任务', async ({ page }) => {
      ...
      await expect(page.locator('.task-card')).toHaveCount(1);
      await expect(page.locator('.task-card__title')).toContainText('已完成任务');
    });
    ...
    test('TC-020: 筛选未完成任务', async ({ page }) => {
      ...
      await expect(page.locator('.task-card')).toHaveCount(1);
      await expect(page.locator('.task-card__title')).toContainText('未完成任务');
    });
  ...
  test.describe('增强功能', () => {
    test('TC-025: 查看任务详情', async ({ page }) => {
      ...
      await expect(page.locator('.task-card__description')).toContainText('任务详细描述');
    });
  ...
  test.describe('v0.2 新增功能', () => {
    ...
    test('创建带截止日期的任务', async ({ page }) => {
      ...
      await page.click('.btn-primary:has-text("添加任务")');
      await expect(page.locator('.task-card__title')).toContainText('有截止日期的任务');
      await expect(page.locator('.task-card__due-date')).toContainText('截止日期');
    });
    ...
    test('创建带标签的任务', async ({ page }) => {
      ...
      await page.click('.btn-primary:has-text("添加任务")');
      await expect(page.locator('.task-card__title')).toContainText('带标签的任务');
